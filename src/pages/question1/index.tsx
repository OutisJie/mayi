...
const Q1: FC = () => {
  ...
  const format = (outputData: any[]) => {
    ...
    for (let i = 0; i < outputData.length; i++) {
      const temp = new Person(outputData[i].name, outputData[i].age, [])
      list.push(temp)
    }
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      // 出到每个人的父母名字
      const currentParentNames = outputData[i].parent ? outputData[i].parent.split(",") : []
